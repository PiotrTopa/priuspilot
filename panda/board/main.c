...
#ifdef PYBOARD
  // Pyboard: force ignition for standalone operation (no ignition sensing hardware)
  ignition_can = true;
  puts("  Pyboard: ignition forced ON\n");
#endif
...
#ifdef PYBOARD
      // Pyboard: no deepsleep, just idle (no ignition wake hardware)
      __WFI();
    }
#else
